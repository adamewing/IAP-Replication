...
# system specific
mapping_tabix = 'ref/map/wgEncodeCrgMapabilityAlign50mer.bed.gz'
nonref_tabix  = 'ref/nonref/IAP.bed.gz'
elsasser_tabix = 'ref/elsasser/elsasser.bed.gz'

mapping = pysam.Tabixfile(mapping_tabix)
nonref  = pysam.Tabixfile(nonref_tabix)
elsasser = pysam.Tabixfile(elsasser_tabix)
...
def output_cluster(cluster, forest, min_size=4):
    if len(cluster) >= min_size:
        ...
        if cluster_chrom not in forest or len(list(forest[cluster_chrom].find(cluster_start, cluster_end))) == 0:
            ...
            if not filter_cluster(cluster) and map_score >= 0.95:
                ...
                if cluster_chrom in elsasser.contigs:
                    els = ['|'.join(te.split()) for te in elsasser.fetch(cluster_chrom, cluster_start, cluster_end)]
                else:
                    els = ['NA']
                ...
                print '#BEGIN'
                print '%s\t%d\t%d\t%s\t%s\t%d\t%0.3f\t%s\t%s' % (cluster_chrom, cluster_start, cluster_end, infer_strand(cluster), bamlist, len(cluster), map_score, els, nr)
                for c in cluster: print c
                print '#END\n'


def cluster(forest, coords, min_size=4, max_spacing=500):
        ...
        if len(cluster) == 0:
            ...
            if c.chrom == cluster[-1].chrom and c.start - cluster[-1].end <= max_spacing:
                ...
                for cluster in subcluster_by_label(cluster):
                    output_cluster(cluster, forest, min_size=min_size)
                cluster = [c]
